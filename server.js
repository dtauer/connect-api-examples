...
server.get('/api/xml', async (req, res) => {
  ...
  if(cookie === '') {
    const login = await axios({url: `${process.env.CONNECT_URL}/api/xml?action=login&login=${process.env.CONNECT_USERNAME}&password=${process.env.CONNECT_PASSWORD}`})
  ...
  if(data.results.status.code !== 'ok' && retries < 3) {
    ...
    if(retries === 3) {
      console.log('Node tried 3 times to log in to your Connect account. Check your credentials in varaibles.env. Also try restarting the node server.')
    }
